        ...
        return (
          <div key={s.n} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center gap-1.5 shrink-0">
              ...
              <span className={`text-2xs text-center max-w-[5.5rem] ${current ? 'text-text font-semibold' : 'text-text-muted'}`}>{s.label}</span>
            </div>
            {i < steps.length - 1 && (
              <div className={`flex-1 h-0.5 mx-1.5 -mt-5 rounded ${s.n < step ? 'bg-success/50' : 'bg-border-subtle'}`} />
            )}
          </div>
        )
      })}
...
function UploadTile({
  ...
  return (
    <Card className={`overflow-hidden transition-colors ${drag ? 'border-accent' : file ? 'border-success/40' : 'border-border/60'}`}>
      <CardContent className="p-4">
        <div className="flex items-center gap-2.5 mb-3">
          <div className={`h-9 w-9 rounded-lg inline-flex items-center justify-center shrink-0 ${file ? 'bg-success/10 text-success' : 'bg-accent/10 text-accent'}`}>
            {file ? <FileCheck2 className="h-4 w-4" /> : <Icon className="h-4 w-4" />}
          </div>
          <div className="min-w-0">
            <div className="text-sm font-medium text-text">{spec.title}</div>
            <div className="text-2xs text-text-muted truncate">{spec.hint}</div>
          </div>
        </div>
        ...
        {!file ? (
          <button
            ...
            className={`w-full rounded-lg border border-dashed ${drag ? 'border-accent bg-accent/5' : 'border-border'} px-3 py-6 flex flex-col items-center gap-1.5 text-center transition-colors hover:border-accent/50 hover:bg-surface-muted/40 focus-ring`}
          >
            <UploadCloud className="h-6 w-6 text-text-muted" />
            <span className="text-xs text-text">Tap to upload <span className="text-text-muted">or drag &amp; drop</span></span>
            <span className="text-2xs text-text-muted">JPG, PNG, WebP, PDF · max 5MB</span>
          </button>
        ) : (
          <div className="rounded-lg border border-border-subtle bg-surface-muted/40 p-2.5 flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="text-xs font-medium truncate text-text">{file.name}</div>
              <div className="text-2xs text-text-muted">{(file.size / 1024 / 1024).toFixed(2)} MB</div>
              {uploading ? (
                ...
                <div className="mt-1 inline-flex items-center gap-1 text-2xs text-success font-medium"><CheckCircle2 className="h-3 w-3" /> Ready to submit</div>
              )}
            </div>
            {!uploading && (
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        variant="hero"
        title="Identity KYC"
        description="Verify your identity (KYC) to unlock payouts on funded accounts."
        icon={ShieldCheck}
        badge={{ label: 'Verification', tone: 'accent' }}
      />
              ...
              <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}>
                <Card className="border-success/40 bg-success/5">
                  <CardContent className="p-6 text-center">
                    <div className="h-14 w-14 rounded-full bg-success/15 text-success inline-flex items-center justify-center mx-auto mb-3">
                      <ShieldCheck className="h-7 w-7" />
                    </div>
                    <div className="font-semibold text-lg text-text">You&apos;re verified</div>
                    <p className="text-sm text-text-muted mt-1 leading-relaxed">Your identity has been approved. Payouts are unlocked on your funded accounts.</p>
                    {kyc?.reviewed_at && <p className="text-2xs text-text-muted mt-2">Approved {fmtDate(kyc.reviewed_at, true)}</p>}
                ...
                <Card className="border-warn/40 bg-warn/5">
                  <CardContent className="p-6">
                    <div className="flex items-start gap-3">
                      <div className="h-11 w-11 rounded-lg bg-warn/15 text-warn inline-flex items-center justify-center shrink-0">
                        <Clock className="h-5 w-5" />
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-semibold text-text">Under review</span>
                          <Badge tone="warn">Pending review</Badge>
                        </div>
                        <p className="text-sm text-text-muted mt-1 leading-relaxed">Thanks — your documents are with our review team. You&apos;ll get an email as soon as a decision is made (usually within 1–2 business days).</p>
                        {kyc?.submitted_at && <p className="text-2xs text-text-muted mt-2">Submitted {fmtDate(kyc.submitted_at, true)}</p>}
                      </div>
                    </div>
            ...
            {status === 'rejected' && (
              <Card className="border-danger/40 bg-danger/5">
                <CardContent className="p-4 flex items-start gap-3">
                  ...
                  <div>
                    <div className="font-medium text-sm text-text">Your previous submission was rejected</div>
              ...
              <div className="grid lg:grid-cols-2 gap-6 items-start">
                  ...
                  <Card className="bg-surface-muted/30">
                    <CardContent className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      <Button onClick={submit} disabled={!allReady || submitting} className="shrink-0 w-full sm:w-auto">
                        {submitting ? <><Loader2 className="h-4 w-4 animate-spin mr-1.5" /> Submitting…</> : 'Submit for verification'}
                      </Button>
                    </CardContent>
                  </Card>
                </div>
                ...
                <Card className="order-1 lg:order-2">
                  <CardContent className="p-5 sm:p-6">
                    <div className="flex items-start gap-3">
                      <div className="h-11 w-11 rounded-lg bg-accent/10 text-accent inline-flex items-center justify-center shrink-0">
                        <ShieldCheck className="h-5 w-5" />
                      </div>
                      <div>
                        <h2 className="font-semibold text-text">Why we verify your identity</h2>
                    ...
                    <div className="space-y-2.5 mt-5">
                      <div className="flex items-start gap-3 rounded-lg border border-border-subtle bg-surface-muted/30 p-3.5">
                        ...
                        <div>
                          <div className="text-sm font-medium text-text">Unlocks payouts</div>
                      ...
                      <div className="flex items-start gap-3 rounded-lg border border-border-subtle bg-surface-muted/30 p-3.5">
                        ...
                        <div>
                          <div className="text-sm font-medium text-text">Bank-grade security</div>
                      ...
                      <div className="flex items-start gap-3 rounded-lg border border-border-subtle bg-surface-muted/30 p-3.5">
                        ...
                        <div>
                          <div className="text-sm font-medium text-text">Fast turnaround</div>
                    ...
                    <div className="mt-5 rounded-lg border border-border-subtle p-4">
                      <div className="text-2xs uppercase tracking-wider text-text-muted font-semibold mb-2.5">Before you start — have these ready</div>
                      ...
                      <p className="text-2xs text-text-muted mt-3 leading-relaxed">Tip: use clear, full-page photos or scans. Blurry, cropped, or expired documents are the most common reason a submission is rejected.</p>
                    </div>
                  </CardContent>
                </Card>
              </div>
