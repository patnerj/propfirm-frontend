...
import type { Notification, NotificationsResp } from '@/types/api'
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">
            All account, challenge, and payout activity in one place.
          </p>
        </div>
        ...
        <CardContent className="p-0">
          {!data ? (
            ...
            <div className="p-12 text-center">
              ...
              <div className="font-medium">{filter === 'unread' ? 'No unread notifications' : 'No notifications yet'}</div>
              <div className="text-xs text-text-muted mt-1">You&apos;ll see account activity here as it happens.</div>
            </div>
          ) : (
            <div>
...
function NotificationRow({ n, index, onRead }: { n: Notification; index: number; onRead: () => void }) {
  ...
  return (
    ...
    >
      <div className={cn('h-9 w-9 rounded-md flex items-center justify-center shrink-0', `bg-${n.type === 'error' ? 'danger' : n.type === 'warning' ? 'warn' : n.type === 'success' ? 'success' : 'info'}-muted`)}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {n.link && (
          <a href={n.link} className="text-2xs text-accent hover:underline mt-1 inline-block">View details →</a>
        )}
      </div>
      {!n.is_read && (
        <button
